...
import se.miman.forge.easyprj.facet.NazgulFacet;

/**
 * Modifies a project to a Nazgul project.
 * Also creates the parent project structure.
 */
@Alias("nazgulprj")
@Help("A plugin that helps to build a Nazgul project in an easy fashion")
@RequiresProject
public class NazgulPrjPlugin implements Plugin
{
	@Inject
   ...
   @SetupCommand
   @Command(value = "setup", help = "Convert project to a Nazgul project")
   public void setup(PipeOut out) {

		if (!project.hasFacet(NazgulFacet.class))
	           event.fire(new InstallFacets(NazgulFacet.class));
	       else
	           ShellMessages.info(out, "Project already an Nazgul project.");
